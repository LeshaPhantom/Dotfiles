...
- **.oh-my-zsh** — большая папка. Если не хотите хранить её в Git, добавьте в `.gitignore`: `.oh-my-zsh/` и уберите запись из `config.yaml` или оставьте только нужные подпапки/файлы через отдельные записи.
